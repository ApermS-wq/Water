...
bot.onText(/\/start/, async (msg) => {
    ...
    const link = `${process.env.DEPLOYED_URL}/?unique_key=${uniqueKey}`;
...
app.get('/get-user-data', async (req, res) => {
    ...
    if (!uniqueKey) {
        return res.status(400).json({ error: 'Unique key is required' });
    }

    try {
        const result = await dbClient.query(
            'SELECT username, balance, buckets FROM users WHERE unique_key = $1',
            [uniqueKey]
        );
